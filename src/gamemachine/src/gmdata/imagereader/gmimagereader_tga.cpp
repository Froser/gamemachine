...
bool TGAImage::read_tga_file(const GMbyte* data, GMuint size)
{
	...
	if (width<=0 || height<=0 || (bytespp!=GRAYSCALE && bytespp!=RGB && bytespp!=RGBA)) {
		gm_error(L"bad bpp (or width/height) value");
		return false;
	}
	...
	else if (10==header.datatypecode||11==header.datatypecode)
	{
		if (!load_rle_data(in))
		{
			gm_error(L"an error occured while reading the data");
			return false;
		}
	}
	else 
	{
		GM_ASSERT(false);
		gm_error(L"unknown file format %d", (int)header.datatypecode);
		return false;
	}
		...
		if (chunkheader<128) {
			...
			for (int i=0; i<chunkheader; i++)
			{
				...
				if (currentpixel>pixelcount) {
					gm_error(L"Too many pixels read");
					return false;
				}
			}
		} else {
...
bool TGAImage::write_tga_file(const char *filename, bool rle) const {
	...
	if (!out.good()) {
		out.close();
		gm_error(L"can't dump the tga file");
		return false;
	}
	if (!rle) {
		out.write((char *)d, width*height*bytespp);
		if (!out.good()) {
			gm_error(L"can't unload raw data");
			out.close();
			return false;
		}
	} else {
		if (!unload_rle_data(out)) {
			out.close();
			gm_error(L"can't unload rle data");
			return false;
		}
	}
