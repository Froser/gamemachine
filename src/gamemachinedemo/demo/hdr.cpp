...
void Demo_HDR::init()
{
...

	{
		...
		gm::GMShader& shader = cube->getShader();
		shader.getMaterial().ks = GMVec3(1);
		shader.getMaterial().kd = GMVec3(10);
		shader.getMaterial().shininess = 99;
		gm::GMToolUtil::addTextureToShader(shader, texture, gm::GMTextureType::Diffuse);
...
void Demo_HDR::handleMouseEvent()
{
	D(d);
...
void Demo_HDR::setDefaultLights()
{
	D(d);
	if (isInited())
	{
...
void Demo_HDR::event(gm::GameMachineEvent evt)
{
	D_BASE(db, Base);
	D(d);
	Base::event(evt);
