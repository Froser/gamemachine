...
namespace
{
	bool DDSHeaderToImageDataHeader(const DDS_FILE_HEADER& header, GMImage::Data* image)
	{
	...
	GLsizei getDDSStride(const DDS_FILE_HEADER& header, GLsizei width)
	{
		if (header.std_header.ddspf.dwFlags == DDS_DDPF_FOURCC &&
			header.std_header.ddspf.dwFourCC == DDS_FOURCC_DX10)
		{
	...
	GLenum getTargetFromDDSHeader(const DDS_FILE_HEADER& header)
	{
	...
	bool loadDDS(const GMbyte* data, GMuint size, GMImage::Data* image)
	{
