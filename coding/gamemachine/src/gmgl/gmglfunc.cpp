...
void GMGL::projection(const linear_math::Matrix4x4& mat, GMGLShaderProgram& shaders, const char* projectionMatrixName)
{
...
void GMGL::frustum(GMfloat left, GMfloat right, GMfloat bottom, GMfloat top, GMfloat n, GMfloat f, GMGLShaderProgram& shaders, const char* projectionMatrixName)
{
	linear_math::Matrix4x4 mat(linear_math::frustum(left, right, bottom, top, n, f));
	projection(mat, shaders, projectionMatrixName);
}

void GMGL::perspective(const linear_math::Matrix4x4& projectionMatrix, GMGLShaderProgram& shaders, const char* projectionMatrixName)
{
	projection(projectionMatrix, shaders, projectionMatrixName);
}

void GMGL::lookAt(const linear_math::Matrix4x4& viewMatrix, GMGLShaderProgram& shaders, const char* viewMatrixName)
{
...
void GMGL::cameraPosition(const CameraLookAt& lookAt, GMGLShaderProgram& shaders, const char* matrixName)
{
...
void GMGL::uniformMatrix4(GMGLShaderProgram& shaders, GMfloat* mat, const char* matrixName)
{
	GLint loc = glGetUniformLocation(shaders.getProgram(), matrixName);
	glUniformMatrix4fv(loc, 1, GL_FALSE, mat);
}

void GMGL::uniformTextureIndex(GMGLShaderProgram& shaders, GMint id, const char* textureName)
{
...
void GMGL::uniformInt(GMGLShaderProgram& shaders, int value, const char* name)
{
	GLint loc = glGetUniformLocation(shaders.getProgram(), name);
	glUniform1i(loc, value);
}

void GMGL::uniformVec3(GMGLShaderProgram& s, GMfloat* value, char* name)
{
...
void GMGL::uniformFloat(GMGLShaderProgram& shaders, GMfloat value, char* name)
{
	GLint loc = glGetUniformLocation(shaders.getProgram(), name);
